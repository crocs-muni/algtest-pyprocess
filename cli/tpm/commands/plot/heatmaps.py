...
def plot_heatmap(
    df: RSADataFrame, device_name: DeviceName, title: PlotTitle, filename: FileName
):
    Heatmap(rsa_df=df, device_name=device_name, title=title).build().save(filename)
...
@click.option("--group-by-tpm", is_flag=True)
...
def heatmaps_grouped(measurements_path, output_path, group_by_tpm, sort, title):
    ...
    MEASUREMENTS_PATH is a directory path containing tpm2-algtest
        ...
        for p, items in processed_profiles:
            ...
            if new.get(dn) is None:
                ...
                for alg_enum, curr_df, _, fname, _ in items:
                    path = p.results[alg_enum].path.replace(measurements_path, "")
                    csv_data.setdefault(
                        (dn, alg_enum),
                        [
                            p.manufacturer,
                ...
                for idx, item in enumerate(items):
                    ...
                    new[dn][1][idx] = (alg_enum, new_df, dn, filename, title_suff)
